...
class PpiTestDataset(BaseDataset):
    ...
    def __getitem__(self, row_idx):
        ...
        res_idx = torch.zeros(total_length, dtype=torch.int)
        original_res_idx = torch.zeros(total_length, dtype=torch.int)
        ...
        res_idx[:target_len] = target_feats.get('res_idx', target_feats['original_res_idx'])
        original_res_idx[:target_len] = target_feats['original_res_idx']
        if binder_len > 0:
            binder_res_idx = torch.arange(1, binder_len + 1, dtype=torch.int)
            res_idx[target_len:] = binder_res_idx
            original_res_idx[target_len:] = binder_res_idx
        ...
        output_feats = {
            ...
            'res_idx': res_idx,
            'original_res_idx': original_res_idx
        }
        return output_feats
...
class PpiScaffoldingTestDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        res_idx[:target_len] = target_feats.get('res_idx', target_feats['original_res_idx'])
        original_res_idx[:target_len] = target_feats['original_res_idx']
        if sampled_binder_length > 0:
            binder_res_idx = torch.arange(1, sampled_binder_length + 1, dtype=torch.int)
            res_idx[target_len:] = binder_res_idx
            original_res_idx[target_len:] = binder_res_idx
