...
def create_full_prot(
    ...
    if residue_index is not None:
        if hasattr(residue_index, "detach"):
            residue_index = residue_index.detach().cpu().numpy()
        residue_index = np.array(residue_index).squeeze()
    ...
    if binder:
        arrays = [chain_index, atom37, atom37_mask, aatype, b_factors]
        if residue_index is not None:
            arrays.append(residue_index)
        swapped = swap_index(chain_index, arrays)
        if residue_index is not None:
            chain_index, atom37, atom37_mask, aatype, b_factors, residue_index = swapped
        else:
            chain_index, atom37, atom37_mask, aatype, b_factors = swapped
        chain_index = np.where(chain_index, 0, 1)
        if residue_index is None:
            residue_index = np.concatenate([
                np.arange(1, sum(chain_index == 0) + 1),
                np.arange(1, sum(chain_index == 1) + 1),
            ])
    else:
        if residue_index is None:
